...
const AlertCard = ({ data, onClose }: AlertCardProps): JSX.Element => {
    ...
    const divStyle = data.type === 'success' ? "bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded absolute top-4 left-0 right-0 text-center mx-auto w-fit max-w-screen" :
        "bg-red-100 border border-red-400 text-red-700 px-8 py-3 rounded absolute top-0 left-0 right-0 text-center mx-auto w-fit max-w-screen";
    ...
    return (
        <div className={divStyle} role="alert">
            ...
            <span onClick={() => { onClose(null) }} className="absolute top-0 bottom-0 right-0 pr-1 py-2.5">
