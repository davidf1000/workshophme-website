...
const ReturnEditModal = ({ formData, setFormData, setShowModal, setActionResult, refreshData }: ReturnEditModalProps): JSX.Element => {
    ...
    const onEdit = async (e: any): Promise<any> => {
        e.preventDefault()
        const { error: err, result: res } = validateReturnForm(formData);
        setError(res);
        if (!err) {
            setLoading(true);
            try {
                ...
                <div className="relative w-auto my-6 mx-auto max-w-md bg-red-200">
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        <div className="flex items-start justify-between p-3 border-b border-solid border-slate-200">
                            <h2 className="text-2xl font-semibold">Return Rent</h2>
                            <button
                        ...
                        <div className="relative px-3 mb-10 flex flex-col my-2 w-full">
                            ...
                            <div className="flex flex-col mb-2">
                                ...
                                />
                                {error.returnName && (
                                    <span className="text-sm text-red-600 mt-1 ml-2">{error.returnName}</span>
                                )}
                            </div>
                            {/* NIM Return */}
                            <div className="flex flex-col mb-2">
                                ...
                                <input
                                    ...
                                    name="returnNim"
                                    value={formData.returnNim}
                                    onChange={(e) => onChange(e)}
                                />
                                {error.returnNim && (
                                    <span className="text-sm text-red-600 mt-1 ml-2">{error.returnNim}</span>
                                )}
                            </div>
...
export default ReturnEditModal;