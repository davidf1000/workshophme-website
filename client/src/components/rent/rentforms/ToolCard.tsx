import { Tool } from '../rent.types';

const ToolCard = ({ tool }: ToolCardProps): JSX.Element => {
  return (
    <>
      <div className="flex px-2 py-4 sm:py-3 sm:px-3 basis-1/2 sm:basis-1/3 md:basis-1/4">
        <div className="flex flex-col bg-slate-50 w-full h-full rounded-xl shadow-lg">
          ...
          <div className="flex flex-col justify-between p-2 w-full max-w-xl">
            <h2 className="font-sans text-sm font-semibold mb-1 mx-auto">
              {tool.name.length > 50
                ? tool.name.substring(0, 50) + '...'
                : tool.name}
            </h2>
            <p className="font-sans font-semibold text-sm mx-auto">
              {tool.priceHour}/jam
            </p>
            <p className="font-sans font-semibold text-sm mx-auto">
              {tool.priceDay}/hari
            </p>
            <div className="flex flex-row justify-between items-end mx-1 my-2">
              {/* logo + */}
              <svg
